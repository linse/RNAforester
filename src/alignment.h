...
protected:
    ...
    virtual unsigned int backtrace(ForestAli<L,AL> &f, CSFPair p, unsigned int &node, int t) = 0;
...
class AlignmentLinear : public Alignment<R,L,AL> {
    ...
    unsigned int backtrace(ForestAli<L,AL> &f, CSFPair p, unsigned int &node, int t);
